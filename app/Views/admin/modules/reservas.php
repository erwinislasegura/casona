  ...
  <div class="admin-table-wrap"><table class="admin-table reservations-table improved-reservations"><thead><tr><th>Solicitud</th><th>Cliente y contacto</th><th>Reserva</th><th>Documentos</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>
    <?php foreach (($reservas ?? []) as $reserva): ?>
      <tr>
        ...
        <td><div class="document-links"><?php if (!empty($reserva['receipt_path'])): ?><a class="receipt-link" href="<?= htmlspecialchars($basePath . $reserva['receipt_path'], ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener">Comprobante</a><?php elseif (!empty($reserva['receipt_name'])): ?><a class="receipt-link" href="<?= $basePath ?>/admin/comprobante/?id=<?= (int)$reserva['id'] ?>" target="_blank" rel="noopener">Comprobante</a><?php else: ?><span>Sin comprobante</span><?php endif; ?><?php if (($reserva['status'] ?? '') === 'approved'): ?><a class="receipt-link is-ticket" href="<?= $basePath ?>/admin/entrada-pdf/?id=<?= (int)$reserva['id'] ?>">Entrada PDF única con QR</a><?php endif; ?></div></td>
        ...
        <td><details class="actions-dropdown"><summary>Opciones</summary><div class="actions-menu">
          <form method="post"><input type="hidden" name="action" value="update_reserva"><input type="hidden" name="reserva_id" value="<?= (int)$reserva['id'] ?>"><input type="hidden" name="status" value="approved"><button class="action-button is-approve" type="submit">Aprobar y emitir entrada única</button></form>
          ...
          <?php if (($reserva['status'] ?? '') === 'approved'): ?><a class="action-button is-ticket" href="<?= $basePath ?>/admin/entrada-pdf/?id=<?= (int)$reserva['id'] ?>">Descargar entrada única con QR</a><?php endif; ?>
